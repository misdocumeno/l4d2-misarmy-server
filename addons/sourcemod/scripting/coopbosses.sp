...
public OnRoundOver(Event event, const char[] name, bool dontBroadcast) {
	g_bIsFinale = false;
...
public Action:Timer_SpawnTank( Handle:timer ) {	
	...
	if( IsTankInPlay() ) {
		...
		g_bHasEncounteredTank = true;
		CPrintToChatAll("{olive}[{default}CB{olive}]{default} Failed to find a spawn for tank in maximum allowed attempts"); 
		return Plugin_Stop; 
	} else {
...
public OnFinaleStart(Event event, const char[] name, bool dontBroadcast) {
	g_bIsFinale = true;
