...
// 	for (int i = 1; i <= MaxClients; i++) {
// 		if (IsClientInGame(i) && L4D2_GetPlayerZombieClass(i) == L4D2ZombieClass_Jockey) {
// 			IsSkeeteable(i);
// 		}
// 	}
// }
...
// TODO: timestamp cambia cuando el jockey vuelve a tocar el piso luego de usar la habilidad.
// talvez podamos usar eso, en OnGameFrame, para saber cuando cayo al piso luego de hacer una habilidad


bool IsSkeeteable(int jockey) {
	...
	if (ability > 0 && IsValidEdict(ability))
	{
		...
		PrintToServer("%N's jockey ability duration = %f, timestamp = %f (current = %f)", jockey, duration, timestamp, GetGameTime());
	}
	return true;
}
