...
// int GetModelClient(int entity) {
// 	for (int i = 1; i <= MaxClients; i++) {
// 		if (g_iModelIndex[i] == entity) {
// 			return i;
// 		}
// 	}
// 	return -1;
...
// 	return (entity > 0 && entity <= MaxClients && IsClientInGame(entity));
// }
...
// TODO: implement this (read all the conversation): https://discord.com/channels/335290997317697536/335290997317697536/1348373891990552686
// TODO: also, try to change the alpha, individually, depending on the player viewing the infected or not
// Action Hook_SetTransmit(int entity, int client)
// {
// 	// if (IsPlayer(entity)) {
// 	// 	PrintToServer("forcefully transmitting %N to %N", entity, client);
// 	// 	return Plugin_Continue;
// 	// }

// 	int player = GetModelClient(EntIndexToEntRef(entity));
// 	char name[32];
// 	GetEntityClassname(entity, name, sizeof(name));

// 	if(DelayWatchGlow_Timer[client] != null) {
// 		return Plugin_Continue;
// 	}

// 	if( g_bSpecCheatActive[client] && !g_bInGame[client] && GetClientTeam(client) == L4D_TEAM_SPECTATOR)
// 	{
// 		// make the glows always visible, not freezing when out of sight, by making the player
// 		// to always transmit. tho it may cause unwanted side effects in players, idk yet...
// 		SetEdictFlags(player, (GetEdictFlags(player) & ~(FL_EDICT_PVSCHECK | FL_EDICT_DONTSEND)) | FL_EDICT_ALWAYS);
// 		// SetEdictFlags(entity, (GetEdictFlags(entity) & ~(FL_EDICT_PVSCHECK | FL_EDICT_DONTSEND)) | FL_EDICT_ALWAYS);

// 		// PrintToServer("[%d] %s: pvscheck: %d, dontsend: %d, edictalways: %d", GetGameTickCount(), name, GetEdictFlags(entity) & FL_EDICT_PVSCHECK != 0, GetEdictFlags(entity) & FL_EDICT_DONTSEND != 0, GetEdictFlags(entity) & FL_EDICT_ALWAYS != 0);
// 		// PrintToServer("[%d] %N: pvscheck: %d, dontsend: %d, edictalways: %d", GetGameTickCount(), player, GetEdictFlags(player) & FL_EDICT_PVSCHECK != 0, GetEdictFlags(player) & FL_EDICT_DONTSEND != 0, GetEdictFlags(player) & FL_EDICT_ALWAYS != 0);
// 	 	return Plugin_Continue;
// 	// } else {
// 		// PrintToServer("[%d] %s and %N does not met some condition", GetGameTickCount(), name, player);
// 	}
