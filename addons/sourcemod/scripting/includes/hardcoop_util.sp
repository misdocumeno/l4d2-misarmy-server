...
stock Float:GetFarthestSurvivorFlow() {
	...
	for (new client = 1; client <= MaxClients; client++) {
        if ( IsSurvivor(client) && IsPlayerAlive(client) ) {
            GetClientAbsOrigin(client, origin);
            new Float:flow = float(GetFlow(origin));
...
stock GetClosestSurvivor( Float:referencePos[3], excludeSurvivor = -1 ) {
	...
	new closestSurvivor = GetRandomSurvivor(1);	
...
stock bool IsInfected(int client) {
	...
	if (GetClientTeam(client) == _:L4DTeam_Infected) return true;
	return false;
}
		...
		if( doUseCommandBot ) {
			...
			if ( !IsValidClient(commandDummy) || IsClientInKickQueue(commandDummy) ) { // Command bot may have been kicked by SMAC_Antispam.smx
			    ...
			    if( IsValidClient(commandDummy) ) {
			    	...
			    	commandDummy = GetRandomSurvivor(1); // wanted to use a bot, but failed; last resort
			    }			
			}
		} else {
			commandDummy = GetRandomSurvivor(1);
		}
