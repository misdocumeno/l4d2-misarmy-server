...
SpawnTimers_OnModuleStart() {
	// Timer
	hSpawnTimeMin = CreateConVar("ss_time_min", "12.0", "The minimum auto spawn time (seconds) for infected", FCVAR_NONE, true, 0.0);
	hSpawnTimeMax = CreateConVar("ss_time_max", "15.0", "The maximum auto spawn time (seconds) for infected", FCVAR_NONE, true, 1.0);
	hSpawnTimeMode = CreateConVar("ss_time_mode", "1", "The spawn time mode [ 0 = RANDOMIZED | 1 = INCREMENTAL | 2 = DECREMENTAL ]", FCVAR_NONE, true, 0.0, true, 2.0);
	hCvarFrequencyBoomerAmbush = CreateConVar("ss_boomer_frequency", "7.0", "Roughly how often to attempt to ambush survivors with a boomer", FCVAR_NONE);
...
SetSpawnTimes() {
	...
	if( fSpawnTimeMin > fSpawnTimeMax ) { //SpawnTimeMin cannot be greater than SpawnTimeMax
		...
		CalculateSpawnTimes(null, "", ""); //must recalculate spawn time table to compensate for min change
	}
}

public CalculateSpawnTimes(ConVar convar, const char[] oldValue, const char[] newValue) {
