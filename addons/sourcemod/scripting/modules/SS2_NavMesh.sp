...
NavMesh_OnModuleStart()
{
	...
	hCVarRearSpawnMaxTrailingDistance = CreateConVar("ss2_rearspawn_max_trailing_distance", "150", "Limit set on ", FCVAR_NONE, true, 0.0);
...
stock void NavMeshSpawn ( const int SpawnQueue[MAXPLAYERS] )
{
	...
	// little fix: just use the highest flow survivor, so the server
	// doesn't fall apart when trying to spawn SI in multiple areas
	int highestFlow = L4D_GetHighestFlowSurvivor();
	...
	{
		if ( thisClient == highestFlow && IsSurvivor(thisClient) && IsPlayerAlive(thisClient) )
		{
							...
							if ( NavMeshArea_GetCenter(indexArea, posArea) ) // returns true if successful
							{
								...
								if ( flowThisArea >= 0 ) // TODO: checking for flow distance invalidates potential spawn areas with no flow distance attached, not sure of ramifications
								{
									if ( (rearSurvivorFlow - flowThisArea) < GetConVarFloat(hCVarRearSpawnMaxTrailingDistance) )
									{
										...
										if ( CheckSpawnConditions(area, highestFlow) ) // check each tile meets our spawn conditions
										{
...
bool CheckSpawnConditions(CNavArea spawn, int clientWeCare)
{
	...
	if ( !IsSpawnStuck(spawn) )
	{
		...
		for ( int thisClient = 1; thisClient <= MAXPLAYERS; ++thisClient )
		{
			// little fix, read line 89
			if ( thisClient == clientWeCare && IsSurvivor(thisClient) && IsPlayerAlive(thisClient) )
			{	
