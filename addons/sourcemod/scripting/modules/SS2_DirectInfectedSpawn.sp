...
int g_iIncrementalBotNames[9];

void ResetIncrementalBotNames() {
	for (int i = 0; i < 9; i++) {
		g_iIncrementalBotNames[i] = 0;
	}
}
...
int CreateInfected(const char[] zomb, float pos[3], float ang[3])
{
	...
	if (StrEqual(zomb, "witch", false) || (g_isSequel && StrEqual(zomb, "witch_bride", false)))
	{
		...
		Format(name, sizeof name, "Witch #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Witch]);
		SetClientInfo(witch, "name", name);
		return witch;
	}
	else if (StrEqual(zomb, "smoker", false))
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Smoker #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Smoker]);
	...
	{
		g_iIncrementalBotNames[L4D2ZombieClass_Boomer]++;
		bot = SDKCall(hCreateBoomer, "Boomer");
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Boomer #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Boomer]);
			SetClientInfo(bot, "name", name);
	...
	else if (StrEqual(zomb, "hunter", false))
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Hunter #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Hunter]);
			SetClientInfo(bot, "name", name);
	...
	else if (StrEqual(zomb, "spitter", false) && g_isSequel)
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Spitter #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Spitter]);
			SetClientInfo(bot, "name", name);
	...
	else if (StrEqual(zomb, "jockey", false) && g_isSequel)
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Jockey #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Jockey]);
			SetClientInfo(bot, "name", name);
	...
	else if (StrEqual(zomb, "charger", false) && g_isSequel)
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Charger #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Charger]);
			SetClientInfo(bot, "name", name);
	...
	else if (StrEqual(zomb, "tank", false))
	{
		...
		if (IsValidClient(bot)) {
			char name[MAX_NAME_LENGTH];
			Format(name, sizeof name, "Tank #%d", ++g_iIncrementalBotNames[L4D2ZombieClass_Tank]);
			SetClientInfo(bot, "name", name);
...
{
	if (!IsValidEntity(infected)) return Plugin_Continue;
	char class[64];
	GetEntityClassname(infected, class, sizeof(class));
	if (!StrEqual(class, "infected", false)) return Plugin_Continue;
