...
void CVS_OnModuleStart()
{
	...
	g_hCvarWarnCvarChanges = CreateConVarEx("warncvarchanges", "1", "Warn when ConVars are changed during a match");
}
...
static void AddCvar(const char[] cvar, const char[] newval)
{
	...
	for (int i = 0; i < iSize; i++) {
		...
		if (strcmp(cvar, cvarBuffer, false) == 0) {
			if (!g_hCvarOverwriteCvars.BoolValue) {
				PrintToServer("cvar %s already set", cvar);
				Debug_LogError(CVS_MODULE_NAME, "Attempt to track ConVar %s, which is already being tracked.", cvar);
				return;
			}

			PrintToServer("[confoglcompmod] Overwriting cvar %s with value %s", cvar, newval);
			strcopy(newEntry.CVSE_newval, CVS_CVAR_MAXLEN, newval);
			CvarSettingsArray.SetArray(i, newEntry, sizeof(newEntry));
			return;
		}
	}

	PrintToServer("setting new cvar %s to %s", cvar, newval);
...
{
	if (bTrackingStarted && g_hCvarWarnCvarChanges.BoolValue) {
