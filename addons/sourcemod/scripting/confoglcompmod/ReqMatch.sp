...
void RM_OnModuleStart()
{
	...
	RM_hAutoLoadLobby	   = CreateConVarEx("match_autoload_lobby", "0", "If this and match_autoload are enabled, it will only load a config if the player connected from a lobby, and not directly with the IP address", _, true, 0.0, true, 1.0);
...
void RM_OnClientConnect(int client)
{
	if (IsFakeClient(client) || !RM_hAutoLoad.BoolValue || RM_bIsAMatchActive)
	{
		return;
	}

	if (RM_hAutoLoadLobby.BoolValue && !L4D_LobbyIsReserved())
	{
		return;
	}
	...
	PrintToServer("[Confogl] Players connected from lobby. Loading %s config...", buffer);
	RM_UpdateCfgOn(buffer);
	RM_Match_Load(true);
}

static void RM_Match_Load(bool instantMapChange = false)
{
	if (RM_bDebugEnabled || IsDebugEnabled())
	{
	...
	if (!RM_bIsMapRestarted && RM_hDoRestart.BoolValue)
	{
		...
		DataPack hDp;
		CreateDataTimer(instantMapChange ? 0.0 : MAPRESTARTTIME, RM_Match_MapRestart_Timer, hDp);
