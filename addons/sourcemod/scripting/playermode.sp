	...
	if( IsSurvivor(client) || IsGenericAdmin(client) ) {
		if( args == 1 ) {
			...
			if( iValue > 0 && iValue <= GetConVarInt(hCvarMaxSurvivors) ) 
			{
				...
				{
					new seconds = CheckBuiltinVoteDelay();
				...
				char title[256];
				Format(title, sizeof title, "Change to %d player mode?", iValue);
...
void PlayerModeVoteActionHandler(Handle vote, BuiltinVoteAction action, int param1, int param2) {
	switch (action) {
...
void PlayerModeVoteResultHandler(Handle vote, int num_votes, int num_clients, const int[][] client_info, int num_items, const int[][] item_info) {
	for (int i = 0; i < num_items; i++) {
		if (item_info[i][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES) {
			if (item_info[i][BUILTINVOTEINFO_ITEM_VOTES] > (num_votes / 2)) {
				char sVotepass[64];
				Format(sVotepass, sizeof(sVotepass), "Changing to %d playermode!", g_iPlayerMode);
