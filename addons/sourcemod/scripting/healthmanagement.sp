...
public OnRespawnHealthChanged(ConVar convar, const char[] oldValue, const char[] newValue) {
	SetConVarInt(hCvarSurvivorRespawnHealth, 100);
}
...
public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (should_leech && victimId && attackerId && IsClientAndInGame(victim) && IsClientAndInGame(attacker))
    {
        if (GetClientTeam(attacker) == _:L4D2Team_Survivor && GetClientTeam(victim) == _:L4D2Team_Infected)
        {
           ...
           new leechedHealth = RoundToFloor(GetConVarFloat(hCvarLeechPercent) * float(damageDone));
...
public ResetSurvivors(Event event, const char[] name, bool dontBroadcast) {
	RestoreHealth();
	ResetInventory();
}
