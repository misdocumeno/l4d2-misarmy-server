					...
					if (!IsFakeClient(attacker))
					{
						...
						if (mode == 1)
						{
							if (shots == 1)
							{
								...
								CreateTimer(0.1, Timer_PrintTopSkeeters, attacker, TIMER_FLAG_NO_MAPCHANGE);
								Skeeted(attacker);
								Skeets[attacker]++;
								Kills[attacker]++;
							}
...
Action Award(Handle timer, int client)
{
	...
	{
		EmitSoundToClient(client, "player/orch_hit_Csharp_short.wav", client, 3, 140, 0, 1.0, timerDeath[client], -1, NULL_VECTOR, NULL_VECTOR, true, 0.0);
...
void PrintTopSkeeters(int client)
{
	...
	{
		if(g_hCvar1v1Separate.BoolValue && Is1v1)
			FormatEx(sBuffer, sizeof(sBuffer), "Top 5 one-shot skeeters (1v1)");
		else
			FormatEx(sBuffer, sizeof(sBuffer), "Top 5 one-shot skeeters");
	}
	else
	{
		if(g_hCvar1v1Separate.BoolValue && Is1v1)
			FormatEx(sBuffer, sizeof(sBuffer), "Top %d skeeters (1v1)", TOP_NUMBER);
		else 
	...
	if (totalskeets)
	{
		...
		panel.DrawText("\n ");
		FormatEx(sBuffer, sizeof(sBuffer), "%d skeets in total%s", totalskeets, (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
		panel.DrawText(sBuffer);
	}
	else
	{
		Format(sBuffer, sizeof(sBuffer), "There are no skeets on this server yet%s", (g_hCvar1v1Separate.BoolValue && Is1v1) ? "(1v1)" : ".");
	}
...
Action Timer_Statistic(Handle timer, int attacker)
{
	...
	if(attacker > 0 && IsClientInGame(attacker))
	{
		...
		if(CvarAnnounce)
		{
			PrintToChat(attacker, "\x01[\x04SM\x01] \x03你 \x04一共有 \x01%d 次空爆%s", skeet, (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
		}
	}
...
void PrintSkeetsToClient(int client)
{
	...
	if (skeet == 1)
	{
		CPrintToChat(client, "[{orange}Skeets{default}] {blue}You have only {olive}1 {blue}skeet%s", (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
	}
	else if (skeet < 1)
	{
		CPrintToChat(client, "[{orange}Skeets{default}] {blue}You don't have any skeet%s", (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
	}
	else
	{
		CPrintToChat(client, "[{orange}Skeets{default}] {blue}You have {olive}%d {blue}skeets%s", skeet, (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
	}
	return;
}
...
void ShowSkeetRank(int client)
{
	...
	CPrintToChat(client, "[{orange}Skeets{default}] {blue}Skeet Ranking: {olive}%d{blue}/{olive}%d{blue}%s", rank, count, (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
}
