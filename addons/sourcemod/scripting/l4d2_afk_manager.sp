...
public void OnPluginStart()
{
	...
	g_cvKickTime = CreateConVar("l4d2_afk_manager_kick_time", "180.0", "After how many seconds spectator players will be kicked from the server if they don't join an empty slots in a team. 0.0=Disabled", FCVAR_NONE);
		...
		switch (team)
		{
			case TEAM_INF, TEAM_SUR:
			{
				...
				if (GetClientAFKTime(i) >= g_fSpecTime)
				{
					...
					CPrintToChat(i, "{red}[{orange}!{red}] {default}You were moved to the {orange}spectator team {default}due to inactivity.");
				}
				else if (GetClientAFKTime(i) >= (g_fSpecTime - 5)) {
					CPrintToChat(i, "{red}[{orange}!{red}] {default}You will be moved to the {orange}spectator team {default}in {olive}%ds {default} for being AFK.", RoundToCeil(g_fSpecTime - GetClientAFKTime(i)));
				}
			}
			case TEAM_SPEC:
			{
				...
				if (g_iTimeAsSpec[i] >= g_fKickTime)
				{
					if (!IsClientInKickQueue(i))
						KickClient(i, "You were kicked for not joining the game");
				} else if (g_iTimeAsSpec[i] > 5 && (RoundToFloor(g_fKickTime - g_iTimeAsSpec[i]) % 10 == 0 || RoundToFloor(g_fKickTime - g_iTimeAsSpec[i]) <= 5)) {
					CPrintToChat(i, "{red}[{orange}!{red}] {default}You will be {red}kicked {default}in {olive}%ds {default} if you don't join a team!", RoundToFloor(g_fKickTime - g_iTimeAsSpec[i]));
				}
			}
			default: {
				// probably loading
...
bool ShouldKickSpectators() {
	if (LibraryExists("readyup") && IsInReady()) return false;
	...
	return g_cvSurvivorLimit.IntValue + g_cvInfectedLimit.IntValue > players;
}
