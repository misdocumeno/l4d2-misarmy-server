...
#define MSGTAG "[{olive}VoteManager{default}]"
...
public void OnPluginStart()
{
	...
	CreateConVar("l4d2_votemanager_version", PLUGIN_VERSION, "Version of VoteManager3 on this server", CVAR_FLAGS|FCVAR_DONTRECORD);
	...
	RegConsoleCmd("sm_votepass", Command_VotePassvote, "Pass a current vote");
	RegConsoleCmd("sm_passvote", Command_VotePassvote, "Pass a current vote");
	RegConsoleCmd("sm_veto", Command_VoteVeto, "Veto a current vote");
	RegConsoleCmd("sm_voteveto", Command_VoteVeto, "Veto a current vote");
	RegConsoleCmd("sm_vetovote", Command_VoteVeto, "Veto a current vote");
	...
	if(argc >= 1)
	{
		...
		if((ClientHasAccess(client, "cooldown_immunity") || iNextVote[client] <= flEngineTime) && VoteStatus == VOTE_NONE)
		{
			...
			if(ClientHasAccess(client, sIssue))
			{
				...
				{
					CReplyToCommand(client, "%s %T", MSGTAG, "Use sm_customvote", client);
					return Plugin_Handled;
				}	
		...
		{
			CPrintToChat(client, "%s %T", MSGTAG, "Conflict", LANG_SERVER);
		...
		{
			CPrintToChat(client, "%s %T", MSGTAG, "Wait", LANG_SERVER, RoundToNearest(iNextVote[client]-flEngineTime));
...
public Action CreateVote(Handle Timer, any client)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			Handle bf = StartMessageOne("VoteStart", i, USERMSG_RELIABLE);
...
*/
public Action VotePass(UserMsg msg_id, Handle bf, const int[] players, int playersNum, bool reliable, bool init)
{
...
byte	team
*/
public Action VoteFail(UserMsg msg_id, Handle bf, const int[] players, int playersNum, bool reliable, bool init)
{
...
public Action Command_VoteVeto(int client, int args)
{
	if(VoteStatus == VOTE_POLLING && ClientHasAccess(client, "veto"))
	{
		...
		else
		{
			...
			CReplyToCommand(client, "%s %T", MSGTAG, "Cant Veto", LANG_SERVER, client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}	
		LogVoteManager("%T", "Vetoed", LANG_SERVER, client);
		CReplyToCommand(client, "%s %T", MSGTAG, "Vetoed", LANG_SERVER, client);
	...
	{
		CReplyToCommand(client, "%s %T", MSGTAG, "No Vote", LANG_SERVER);
...
public Action Command_VotePassvote(int client, int args)
{
	if(VoteStatus == VOTE_POLLING && ClientHasAccess(client, "pass"))
	{
		...
		{
			LogVoteManager("%T", "Cant VetoPass", LANG_SERVER, client);
			CReplyToCommand(client, "%s %T", MSGTAG, "Cant Pass", LANG_SERVER, client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}
		LogVoteManager("%T", "Passed", LANG_SERVER, client);
		CReplyToCommand(client, "%s %T", MSGTAG, "Passed", LANG_SERVER, client);
