...
bool g_bCvar_RandomFieldColorInf;
bool g_bCvar_Sprite;
bool g_bCvar_RandomSpriteColor;
// bool g_bCvar_RandomSpriteColorInf;
...
char g_sCvar_SpriteColorInf[12];
...
public void OnPluginStart()
{
    ...
    g_hCvar_FieldColorInf      = CreateConVar("l4d_spot_marker_field_color_inf", "255 255 0", "Beacon field color for infected team.\nUse \"random\" for random colors.\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", CVAR_FLAGS);
    ...
    g_hCvar_SpriteColorInf     = CreateConVar("l4d_spot_marker_sprite_color_inf", "255 255 0", "Sprite color for infected team.\nUse \"random\" for random colors.\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", CVAR_FLAGS);
    ...
    g_hCvar_FieldColorInf.AddChangeHook(Event_ConVarChanged);
    ...
    g_hCvar_SpriteColorInf.AddChangeHook(Event_ConVarChanged);
...
void GetCvars()
{
    ...
    g_hCvar_FieldColorInf.GetString(g_sCvar_FieldColorInf, sizeof(g_sCvar_FieldColorInf));
    TrimString(g_sCvar_FieldColorInf);
    g_bCvar_RandomFieldColorInf = StrEqual(g_sCvar_FieldColorInf, "random", false);
    g_iCvar_FieldColorInf = ConvertRGBToIntArray(g_sCvar_FieldColorInf);
    ...
    g_hCvar_SpriteColorInf.GetString(g_sCvar_SpriteColorInf, sizeof(g_sCvar_SpriteColorInf));
    TrimString(g_sCvar_SpriteColorInf);
    // g_bCvar_RandomSpriteColorInf = StrEqual(g_sCvar_SpriteColorInf, "random", false);
...
{
    static int lastUse[MAXPLAYERS+1], lastZoom[MAXPLAYERS+1];

    // SHIFT + E or MOUSE3
    if (
        ((buttons & IN_SPEED) && (buttons & IN_USE) && lastUse[client] != view_as<int>((buttons & IN_SPEED) && (buttons & IN_USE))) ||
        (buttons & IN_ZOOM) && lastZoom[client] != (buttons & IN_ZOOM))
    {
        if (!g_bCvar_Enabled)
            return;
    ...
    lastUse[client] = (buttons & IN_SPEED) && (buttons & IN_USE);
    lastZoom[client] = buttons & IN_ZOOM;
}
...
void CreateSpotMarker(int client)
{
    ...
    if (g_bCvar_Field)
    {
        ...
        int color[4];
        if (clientTeam == TEAM_INFECTED ? g_bCvar_RandomFieldColorInf : g_bCvar_RandomFieldColor)
        {
            if (g_bCvar_SyncRandomColor)
            {
        ...
        {
            color[0] = clientTeam == TEAM_INFECTED ? g_iCvar_FieldColorInf[0] : g_iCvar_FieldColor[0];
            color[1] = clientTeam == TEAM_INFECTED ? g_iCvar_FieldColorInf[1] : g_iCvar_FieldColor[1];
            color[2] = clientTeam == TEAM_INFECTED ? g_iCvar_FieldColorInf[2] : g_iCvar_FieldColor[2];
            color[3] = g_iCvar_FieldAlpha;
        }
    ...
    if (g_bCvar_Sprite)
    {
        ...
        {
            color = clientTeam == TEAM_INFECTED ? g_sCvar_SpriteColorInf : g_sCvar_SpriteColor;
        }
