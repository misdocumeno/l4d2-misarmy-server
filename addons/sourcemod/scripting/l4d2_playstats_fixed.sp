    ...
    if ( IS_VALID_SURVIVOR(attacker) && IS_VALID_INFECTED(victim) )
    {
        ...
        if ( zClass >= ZC_SMOKER && zClass <= ZC_CHARGER )
        {
            ...
            switch ( weaponType )
            {
                ...
                case WPTYPE_SMG: {     storeA = plyHitsSmg;     storeB = plyHitsSISmg;      storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsSmg : view_as<strPlayerData>(-1); }
                case WPTYPE_SNIPER: {  storeA = plyHitsSniper;  storeB = plyHitsSISniper;   storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsSniper : view_as<strPlayerData>(-1); }
                case WPTYPE_PISTOL: {
                        storeA = plyHitsPistol;  storeB = plyHitsSIPistol;   storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsPistol : view_as<strPlayerData>(-1);
...
public Action: Event_InfectedHurt ( Handle:event, const String:name[], bool:dontBroadcast )
{
    ...
    switch ( weaponType )
    {
        ...
        case WPTYPE_SMG: {     storeA = plyHitsSmg;     storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsSmg : view_as<strPlayerData>(-1); }
        case WPTYPE_SNIPER: {  storeA = plyHitsSniper;  storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsSniper : view_as<strPlayerData>(-1); }
        case WPTYPE_PISTOL: {
                storeA = plyHitsPistol;  storeC = ( hitgroup == HITGROUP_HEAD ) ? plyHeadshotsPistol : view_as<strPlayerData>(-1);
                // incapped: don't count hits
                if ( IsPlayerIncapacitated(attacker) ) { storeA = -1; }
            }
    }
...
stock ResetStats ( bool:bCurrentRoundOnly = false, iTeam = -1, bool: bFailedRound = false )
{
    ...
    else
    {
        if ( iTeam == -1 ) {
            for ( k = 0; k <= MAXRNDSTATS; k++ ) {
                if ( bFailedRound && k == view_as<int>(rndRestarts) ) { continue; }
...
char[] GetMVPChatString( bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
    decl String: printBuffer[1024];
    decl String: tmpBuffer[512];
...
char[] GetFunFactChatString( bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
    decl String: printBuffer[1024];
