...
ConVar
	...
	cvar_unfreezeAI,
	cvar_blockSafeDoor,
	cvar_closeBlockedDoor;

public Plugin myinfo =
{
	name        = "L4D2 No Tank Rush",
	author      = "Jahze, vintik, devilesk, Sir, Mis",
	version     = "1.1.4",
	description = "Stops distance points accumulating whilst the tank is alive, with the option of unfreezing distance on reaching the Saferoom or blocking its door"
};
...
public void OnPluginStart()
{
	...
	cvar_blockSafeDoor    = CreateConVar("l4d_no_tank_rush_block_safe_door", "0", "Block the end saferoom door while the Tank is still up", _, true, 0.0, true, 1.0);
	cvar_closeBlockedDoor = CreateConVar("l4d_no_tank_rush_close_safe_door", "0", "Close the end saferoom door while the Tank is still up", _, true, 0.0, true, 1.0);
...
void FreezePoints(bool show_message = false)
{
	if (!bTankAlive)
	{
		...
		if (cvar_blockSafeDoor.BoolValue)
		{
			int door = L4D_GetCheckpointLast();
			if (door == -1) return;
			int doorFlags = GetEntProp(door, Prop_Send, "m_spawnflags");
			SetEntProp(door, Prop_Send, "m_spawnflags", doorFlags | DOOR_FLAG_IGNORE_USE);
			if (cvar_closeBlockedDoor.BoolValue)
